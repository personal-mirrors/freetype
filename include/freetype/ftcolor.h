...
#define FT_PALETTE_FOR_DARK_BACKGROUND   0x02
   ...
   *   glyphs may optionally define a ClipBox for aiding allocation or
   *   defining a maximum drawable region.  Use
   *   @FT_Get_Color_Glyph_ClipBox to retrieve it.
   *
   * @fields:
   *   bottom_left ::
   ...
   *   and fill the `clip_box` parameter with the 'COLR' v1 ClipBox
   ...
   *     clip box is computed taking scale and transformations
   *     configured on the @FT_Face into account.  @FT_ClipBox contains
   *     @FT_Vector values in 26.6 format.  Reset scale to units-per-em
   *     and remove transforms configured using @FT_Set_Transform in
   *     order to retrieve clip box in font units.
   *
   * @return:
