...
#define FT_PALETTE_FOR_DARK_BACKGROUND   0x02
   ...
   *
   *   The enumeration values losely correspond with the format numbers of the
   *   specification: FreeType will always return a fully specified Paint
   *   structure for the Transform, Translate, Scale, Rotate, and Skew table
   *   types even though the specification has different formats depending on
   *   whether or not a center is specified or the scale is uniform in x and
   *   y~direction or not etc. Also, only non-variable format identifiers are
   *   listed in this enumeration; as soon as support for variable 'COLR' v1
   *   fonts is implemented, interpolation is performed dependent on axis
   *   coordinates, which are configured on the @FT_Face through
   *   @FT_Set_Var_Design_Coordinates.  This implies that always static, readily
   *   interpolated values are returned in the Paint structures.
   *
   * @since:
   ...
   *   tables.  Used for scaling downstream paints by a given x and
   *   y~scale, with a given center. This structure is used for all
   *   `PaintScale*` types that are part of specification and fields of
   *   this structure are filled accordingly. If there is a center, the
   *   center values are set, otherwise they are set to the zero
   *   coordinate. If the source font file has a `PaintScaleUniform*`
   *   then the scale values are set accordingly to the same value.
   *
   * @fields:
   *   paint ::
   *     An @FT_OpaquePaint object referencing the paint that is to be
