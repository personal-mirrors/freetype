...
#define FT_PALETTE_FOR_DARK_BACKGROUND   0x02
   ...
   *
   * @description:
   *   This iterator object is needed for @FT_Get_Colorline_Stops.  It
   *   keeps state while iterating over the stops of an @FT_ColorLine,
   *   representing the `ColorLine` struct of the v1 extensions to
   *   'COLR', see
   *   'https://github.com/googlefonts/colr-gradients-spec'. Do not
   *   manually modify fields of this iterator.
   *
   * @fields:
   *   num_color_stops ::
   ...
   *
   *   read_variable ::
   *     A boolean keeping track of whether variable color lines are to
   *     be read. Set by @FT_Get_Paint.
   *
   * @since:
   *   2.11 -- **currently experimental only!**  There might be changes
   *   without retaining backward compatibility of both the API and ABI.
