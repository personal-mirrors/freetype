...
#define FT_COMPONENT  cffgload
                 ...
                 FT_Int32       load_flags )
  {
    ...
    {
      /*
       * We load the SVG document and try to grab the advances from the table.
       * For the bearings we rely on the presetting hook to do that.
       */
      FT_Short      dummy;
      FT_UShort     advanceX;
      FT_UShort     advanceY;
      ...
      if ( !error )
      {
        ...
        /* if the horizontal advance or the vertical advances are not present in the table
         * that's a problem with the font, the standard requires that they have it, but we
         * will be graceful and calculate ourselves in the vertical case.
         */
        ...
        advanceX = FT_MulDiv( advanceX, glyph->root.face->size->metrics.x_ppem,
                              glyph->root.face->units_per_EM );
        advanceY = FT_MulDiv( advanceY, glyph->root.face->size->metrics.y_ppem,
                              glyph->root.face->units_per_EM );
