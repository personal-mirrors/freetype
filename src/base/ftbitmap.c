...
#define FT_COMPONENT  bitmap
                   ...
                   FT_Fixed    yslant )
  {
    ...
    if ( !bitmap || !bitmap->buffer || xslant > 0x10000 || xslant < 0 || yslant != 0)
      return FT_THROW( Invalid_Argument );
    ...
    if ( bitmap_top > 0 )
    {
      above_height = bitmap_top-1;
      above_slant  = ft_bitmap_slant_get_offset( above_height, xslant );
    }
    else
    {
      above_height = 0;
     ...
     * rows = top: else is OK.
     * rows < top: else is OK. */
    if ( bitmap->rows > bitmap_top )
    {
      below_height = bitmap->rows - bitmap_top;
      below_slant  = ft_bitmap_slant_get_offset( below_height, xslant );
    }
    else
    {
      below_height = 0;
    ...
    if ( above_height > 0 )
    {
      ...
      else
      {
        ...
        block_start = bitmap_top - bitmap->rows;
      }
    ...
    if ( bitmap->rows >= bitmap_top )
    {
      ...
      {
        offset = below_slant - ft_bitmap_slant_get_offset( j + 1, xslant );
