...
#define GX_DT_DELTA_RUN_COUNT_MASK  0x3FU
                         ...
                         FT_UInt          innerIndex )
  {
     ...
     * "Fixed is a 32-bit (16.16) type and, in the general case,
     *  requires 32-bit deltas. As described above, the DeltaSet record
     *  can accommodate deltas that are, logically, either 16-bit or
     *  32-bit. When scaled deltas are applied to Fixed values, the
     *  Fixed value is treated like a 32-bit integer."
     * Use FT_MulAddFix which internally uses 64-bit precision and thus can
     * handle deltas ranging from small 8-bit deltas to 32-bit deltas that are
