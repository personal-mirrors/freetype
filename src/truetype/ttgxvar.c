...
#define ALIGN_SIZE( n ) \
                   ...
                   FT_Bool    set_design_coords )
  {
    ...
    if ( !face->is_cff2 && !blend->glyphoffsets ) {
      FT_SET_ERROR( ft_var_load_gvar( face ) );
      /* A missing gvar table is acceptable, for example for fonts that only
         vary metrics information or COLRv1 PaintVar* tables, but an incorrect
         SFNT table offset or size for gvar or an inconsistent gvar table is
         not. Handle return code specifically. */
