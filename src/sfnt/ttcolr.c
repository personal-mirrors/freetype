...
#define VARIABLE_COLRV1_ENABLED                                         \
  ...
  typedef struct  Colr_
  {
    ...
    /* Item Variation Store for variable COLRv1. */
...
#define FT_COMPONENT  ttcolr
                     ...
                     FT_Stream  stream )
  {
    ...
    if ( colr->version == 1 )
    {
           ...
           VARIABLE_COLRV1_ENABLED )
      {
        ...
        if ( var_store_offset ) {
          /* If variation info has not been initialised yet, try doing so,
           * otherwise loading variation store will fail, as it requires access
           * to blend for checking number of axes. */
          if ( !face->blend )
          {
  ...
  InvalidTable:
    ...
    if ( VARIABLE_COLRV1_ENABLED )
    {
      ...
      mm->done_delta_set_idx_map( FT_FACE( face ), &colr->delta_set_idx_map );
      mm->done_item_var_store( FT_FACE( face ), &colr->var_store );
    }
#endif
