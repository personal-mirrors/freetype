...
#define FT_COMPONENT  ttcolr
                               ...
                               FT_ColorStopIterator  *iterator )
  {
    ...
    /* Calculate the start pointer for the last to-be-read (Var)ColorStop and
     * check whether we can read a full (Var)ColorStop at that position by
     * comparing it to the position that's the size of of one (Var)ColorStop
     * before the end of the COLR table. */
    last_entry_p = iterator->p + ( iterator->num_color_stops - 1 -
                                   iterator->current_color_stop ) *
                                     entry_size;
    if ( iterator->p < colr->paints_start_v1                                  ||
         last_entry_p > (FT_Byte*)colr->table + colr->table_size - entry_size )
      return 0;
